...
def _sem_acento(texto):
    """Minusculas e sem acento, para comparar assunto de forma tolerante."""
    txt = unicodedata.normalize("NFKD", texto or "")
    return txt.encode("ascii", "ignore").decode("ascii").lower()


def assunto_no_escopo(subject):
    """True se o assunto casa o filtro de escopo. Funcao PURA.

    Compara sem acento e sem caixa, para que "Devolução", "DEVOLUCAO" e
    "devolucao" casem igual - o cliente digita de qualquer jeito.

    ASSUNTO_FILTRO vazio desliga o filtro (processa tudo).
    """
    filtro = settings.ASSUNTO_FILTRO
    if not filtro:
        return True
    return _sem_acento(filtro) in _sem_acento(subject)
...
def processar_mensagem(message_id):
    ...
    # 1. Buscar o e-mail no Graph (metadados primeiro; anexo so se for do escopo)
    try:
    ...
    # 2. Trava de escopo. Antes de baixar anexo: e-mail fora do fluxo nao deve
    # consumir download de bytes nem chamada ao Vertex, e MUITO menos receber
    # resposta automatica. Sai em silencio - marketing e notificacao nao merecem
    # alerta.
    if not assunto_no_escopo(subject):
        log.info("mensagem %s fora do escopo (assunto=%r nao contem %r); ignorada",
                 message_id, subject, settings.ASSUNTO_FILTRO)
        return

    try:
        attachments = graph_client.get_attachments(message_id)
    except Exception:
        log.exception("falha ao buscar anexos de %s no Graph", message_id)
        alertar("Falha ao buscar anexos no Graph",
