...
#
# SEM DEFAULT, DE PROPOSITO: o resource name carrega o numero do projeto GCP, e
# por decisao do projeto e tratado como valor sensivel. Vem SEMPRE do ambiente -
# .env local ou painel do Railway - nunca de codigo versionado.
...
def ensure_credentials():
    ...
    """
    # Confere que o arquivo EXISTE. Devolver um caminho quebrado faria o boot
    # passar e a falha aparecer so na primeira chamada real ao GCP - foi
    # exatamente o que aconteceu no primeiro deploy no Railway, onde a variavel
    # herdada do .env local apontava para uma chave que nao existe no container.
    caminho_env = os.environ.get("GOOGLE_APPLICATION_CREDENTIALS", "").strip()
    if caminho_env:
