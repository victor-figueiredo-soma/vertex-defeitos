...
Uso:
  uv run python testar_endpoint.py
  uv run python testar_endpoint.py --pasta "C:/caminho" --estrategias generico
"""
...
def resumo(linhas):
    ...
    print()
    print("  --- politica de resposta do app (CONFIDENCE_THRESHOLD="
    ...
    for est in ("generico", "correto", "errado"):
        ...
        if not sub:
            continue
        auto = sum(1 for r in sub
                   if processor.decidir_acao(r)[0] == "responder")
        print(f"    {est:9} {auto}/{len(sub)} responderiam automatico ao cliente")
